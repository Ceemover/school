...
void	draw_background(t_all *all)
{
	...
	h = -1;
	while (!all->txtrs.c_fl && ++h < (all->h / 2) + all->plr.pitch)
	{
		w = -1;
		while (++w < all->w)
			my_mlx_pixel_put(all, w, h, all->txtrs.c);
	}
	h = all->h / 2 + all->plr.pitch;
	while (!all->txtrs.f_fl && h < all->h)
	{
		w = -1;
		while (++w < all->w)
...
void	draw_wall(t_all *all, int x)
{
	...
	all->ray.tex_pos = (all->ray.draw_start - all->plr.pitch -
		(all->plr.pos_z / all->ray.perp_wall_dist[x]) -
		all->h / 2 + all->ray.line_height / 2) * all->ray.step;
	if (all->ray.tex_pos < 0)
		all->ray.tex_pos = 0;
	while (y < all->ray.draw_end)
	{
		loop_draw_walls(all, x, y);
		y++;
	}
}
