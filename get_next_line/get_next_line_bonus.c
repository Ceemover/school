...
/*   Updated: 2020/08/09 09:28:21 by dbolg            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

static int	ft_exit(char **tail)
{
	int fd;
...
static int	n_in_tail(char **head, char **tail, char **buf)
{
	int		nl;
	char	*tmp;
	...
	*(*tail + nl) = '\0';
	...
	{
		if (!(*tail = ft_strjoin((*tail + nl + 1), NULL)))
		{
			f_ree(buf, &tmp, head);
			return (ft_exit(tail));
		}
...
static int	n_in_buf(char **head, char **tail, char **buf)
{
	int		nl;
	char	*tmp;
	...
	{
		if (!(*tail = ft_strjoin((*buf + nl + 1), NULL)))
		{
...
static int	buf_mng(char **head, char **tail, char **buf, int buffsize)
{
	int		i;
	char	*tmp;
...
int			get_next_line(int fd, char **head)
{
	...
	if (fd < 0 || BUFFER_SIZE < 1 || (read(fd, NULL, 0) == -1) || !head
		...
		return (ft_exit(tail));
	*head = NULL;
	if (tail[fd] && (find_n(tail[fd], ft_strlen(tail[fd])) > -1))
	...
	if (!*head)
	{
		if (!(*head = ft_strjoin(NULL, NULL)))
			return (f_ree(&buf, &buf, &buf) ? ft_exit(tail) : -1);
	}
